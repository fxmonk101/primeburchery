...
/**
 * Check admin role via RPC — uses the SECURITY DEFINER `has_role` postgres
 * function so no service-role key is needed on the client.
 */
async function checkIsAdmin(userId: string): Promise<boolean> {
  try {
    ...
    if (error) {
      // Fallback: try querying user_roles directly (works if RLS allows it)
...
function AdminLoginPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    // Step 2 — verify admin role via client-side RPC (no service-role key needed)
